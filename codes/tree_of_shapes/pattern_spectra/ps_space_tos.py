...
def reconstruct_nodes_tos(tree,altitudes,node):
    ...
    return res
def filtermax(im,threshold):
    graph = hg.get_4_adjacency_graph(res11.shape)
    treeps, altitudesps = hg.component_tree_max_tree(graph,im)
    area = hg.attribute_area(treeps)
    filtered = hg.reconstruct_leaf_data(treeps, altitudesps, area < threshold)
    return filtered
def subtractive_rule(tree, altitudes, node):
    node=np.array(node,dtype=int)
    ...
    res=hg.reconstruct_leaf_data(tree,altitudes)
    return res 
#data pleiades   
...
#filtering
filtered1=filtermax(res11,45)
filtered2=filtermax(res21,15)


#extract all bins first image
bins=np.nonzero(filtered1)
bins=np.array(bins)

#extract all bins second image
bins2=np.nonzero(filtered2)
